...
from bamt_special.builders.builders_base import ParamDict
from bamt_special.builders.hc_builder import HCStructureBuilder
...
class BaseNetwork(object):
    ...
    def fit_parameters(self, data: pd.DataFrame, user: str,
                       dropna: bool = True):
        """
        ...
        def worker_gross(node):
            return node.fit_parameters(data, user=user)
        ...
        for node in self.nodes:
            if node.type in ["Discrete", "MixtureGaussian", "ConditionalMixtureGaussian",]:
                future = pool.submit(worker_klein, node)
            else:
                future = pool.submit(worker_gross, node)
